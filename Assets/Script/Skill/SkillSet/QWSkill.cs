...
namespace SkillSystem
{
    public class QWSkill : Skill
    {
        ...
        private void FireLaser(Vector3 startPosition, Vector2Int targetGridPosition)
        {
            ...
            if (tileGrid == null)
            {
                Debug.LogError("QWSkill: TileGrid reference is null when trying to fire laser!");
                return;
            }
            ...
            Debug.Log($"QW Skill: Fired laser toward {targetGridPosition}");
        }
        
        private GameObject CreateBasicLaser()
        {
            // Create a simple laser GameObject
            GameObject laser = new GameObject("QW_Laser");
            ...
            foreach (RaycastHit2D hit in hits)
            {
                ...
                if (hit.collider.CompareTag("Enemy"))
                {
                    ...
                    if (enemy != null)
                    {
                        ...
                        Debug.Log($"QW Skill: Dealt {damage} damage to enemy at position {hit.point}");
                ...
                else if (hit.collider.CompareTag("Obstacle"))
                {
                    ...
                    if (destructible != null)
                    {
                        ...
                        Debug.Log($"QW Skill: Dealt {damage} damage to obstacle at position {hit.point}");
                    }
                }
            }
        }
