...
public class QWSkill : Skill
{
    [Header("QW Skill Settings")]
    ...
    private void Awake()
    {
        ...
        if (tileGrid == null)
        {
            Debug.LogError("QWSkill: Could not find TileGrid in the scene!");
        }
        
        playerCrosshair = FindObjectOfType<PlayerCrosshair>();
        if (playerCrosshair == null)
        {
            Debug.LogError("QWSkill: Could not find PlayerCrosshair in the scene!");
        }
    ...
    private IEnumerator DropProjectileAndCrackTiles(Vector2Int targetPosition, Vector3 targetWorldPos)
    {
        ...
        if (skillProjectilePrefab != null)
        {
            ...
            // Optional: Add a trail effect or particles to the projectile
            ...
            while (projectile != null && Time.time - startTime < journeyLength / projectileDropSpeed)
            {
                ...
                if (projectile != null)
                {
                    ...
                    // Optional: Rotate the projectile as it falls
    ...
    private IEnumerator DelayedTileEffect(Vector2Int position, float delay)
    {
        ...
        // Check if this position is the crosshair target position
        bool isCrosshairPosition = false;
        if (playerCrosshair != null)
        {
            isCrosshairPosition = playerCrosshair.GetTargetGridPosition() == position;
        }
        
        // Check for enemies at this position
        bool enemyFound = CheckForEnemyAtPosition(position);
        
        // If an enemy is at the crosshair position, immediately break the tile
        if (enemyFound && isCrosshairPosition)
        {
            BreakTile(position);
        }
        else
        {
            // Otherwise, deal damage and crack the tile
            DealDamageAtPosition(position);
            
            // Mark tile as cracked
            CrackTile(position);
        }
    }
    ...
    private void DealDamageAtPosition(Vector2Int position)
    {
        ...
        // If no enemy, still crack the tile
        if (!enemyFound && !crackedTilePositions.Contains(position))
        {
    ...
    private IEnumerator MonitorCrackedTiles()
    {
        ...
        while (true)
        {
            // Check all game objects with player movement component
